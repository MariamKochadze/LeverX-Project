...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const basicSearchInput: HTMLInputElement =
    document.querySelector('#basic-input')!;
  const basicSearchBtn: HTMLElement = document.querySelector(
    '#basic-options .search-btn'
  )!;
  const advancedSearchForm: HTMLElement =
  ...
  const gridViewBtn = createViewButton('grid', gridIcon);
  const listViewBtn = createViewButton('list', listIcon);
  ...
  gridViewBtn.addEventListener('click', () =>
    toggleView(gridViewBtn, listViewBtn, 'none')
  );
  listViewBtn.addEventListener('click', () =>
    toggleView(listViewBtn, gridViewBtn, 'flex')
  );
  ...
  const basicSearch = (searchTerm: string) => {
    ...
    const currentView = (
      document.querySelector('.view-toggle.active')! as HTMLElement
    ).dataset.view;
    if (currentView) {
  ...
  const advancedSearch = (formData: UserFormData) => {
    ...
    const currentView = (
      document.querySelector('.view-toggle.active')! as HTMLDivElement
    ).dataset.view;
    if (currentView) {
      renderUsers(filteredUsers, currentView);
    }
  ...
  advancedSearchForm.addEventListener('submit', (e) => {
    ...
    const formData = {
      ...
      building: (document.querySelector('#building')! as HTMLInputElement)
        .value,
      room: (document.querySelector('#room')! as HTMLInputElement).value,
      department: (document.querySelector('#department')! as HTMLInputElement)
        .value,
    };
    ...
    if (users.length === 0) {
      ...
      <div class="not-found search">
          <div class="not-found__content">
              <img src="${notFoundIcon}" alt="Not Found Icon" class="not-found__image">
              <h2 class="not-found__title">404 USERS NOT FOUND</h2>
      ...
      card.innerHTML = `
                <div class="card-header">
                    <div class="avatar-container">
                        <img src="${user.user_avatar.replace('./assets/', '/src/assets/')}" alt="${user.first_name}'s Avatar" class="avatar" />
                    </div>
                    <div class="user-info">
                ...
                <div class="details">
                    <div class="detail-row">
                        <img src="${workingIcon}" alt="Department Icon" class="info-icon" />
                        <span class="detail-text">${user.department}</span>
                    </div>
                    <div class="detail-row">
                        <img src="${noteIcon}" alt="Room Icon" class="info-icon" />
                        <span class="detail-text">${user.room}</span>
                    </div>
                </div>`;

      card.addEventListener('click', () => {
        window.location.href = `/user-details.html#/users/${user.id}`;
      });
